...
def _parse_deps_from_output(output):
    try:
        ...
        deps_info = ' '.join(deps_section_lines).split()

        if len(deps_info) % 6 != 0:
            raise Exception(
                "Dependencies info is not in 6 element chunks. "
                "Could not parse."
            )

        # Split up the data in to 6 element arrays
        info_chunks = [deps_info[i:i+6] for i in range(0, len(deps_info), 6)]
...
def get_needed_dependencies(package):
    ...
    try:
        ...
        info_chunks = _parse_deps_from_output(output)

        for update_info in info_chunks:
            name = update_info[0]
            arch = update_info[1]
            repo = update_info[3]
