...
class OperationManager():
    ...
    def reboot_op(self, operation):
        ...
        # This offsets the reboot by 15 seconds so that the agent check in
        # can occur and dump the operations to file.
        time.sleep(15)

        self._system_reboot(operation.reboot_delay_seconds / 60)

        # Sleeping thread for 2 minutes more than initial, if the computer
        # doesn't reboot by then, send back failed result.
    ...
    def shutdown_op(self, operation):
        ...
        # Sleeping thread for 2 minutes more than initial, if the computer
        # doesn't shutdown by then, send back failed result.
